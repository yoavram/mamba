...
my.labeller <- function(var, value){
  value <- as.character(value)
  if (var=="N") {
    value <- paste("Population size = ", as.character(value))    
  } else if (var=="r") {
    value[value==0] = "Asexual"
    value[value==0.003]  = "Sexual"    
  }
  return(value)
}

g = ggplot(ddf, aes(x=mutator, y=tick1.mean)) + 
  facet_grid(facets=r~N, labeller=my.labeller) +
  labs(x='Mutator', y='Log avg. time to loss of fittest genotype')
g2 = g + geom_bar(aes(fill=mutator), stat="identity") + 
  scale_y_log10() +
  scale_fill_manual(values=c('#377EB8','#E41A1C','#4DAF4A'), guide="none") +
  geom_errorbar(aes(ymax=tick1.mean+tick1.se, ymin=tick1.mean-tick1.se, width=0.2))
g2$data <- subset(g2$data,N!=1000 & pi!=2)
g2

ggsave(filename="first.click_07052014.png", plot=g2, width=4, height=6)
