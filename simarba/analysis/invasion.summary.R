...
setwd('D:\\workspace\\mamba\\simarba\\analysis')

fname <- "invasion_26_05_2013"
df <- read.csv(paste0('..\\..\\output\\',fname,".csv.gz"))
df <- df[df$r < 0.3,]
...
agg.df <- subset(agg.df.all, beta < 1 & mu == 0.003 & envch_str == 4 & in_tau != 20 & in_tau != 1)
...
q4 = q3 + scale_fill_brewer(palette="Set1", name="invader")
q4 + labs(y="# samples") + theme(axis.text.x  = element_text(angle=45, vjust=0.5)) + 
  scale_x_discrete(labels = function(x){format(as.numeric(x))})
...
q9 = q8 + theme(axis.text.x  = element_text(angle=45, vjust=0.5)) 
q10 = q9 + scale_x_discrete(labels = function(x){format(as.numeric(x), scientific=T)})
q10

ggsave(filename=paste0(fname,'.mean_invasion_Success.pdf'), plot=q10, height=8.27, width=11.69)
...
q9 = q8 + scale_x_discrete(labels = function(x){format(as.numeric(x), scientific=T)})
q9

ggsave(filename=paste0(fname,'_SIM_with_r.tiff'), plot=q9, width=8.27, height=11.69, dpi=100)
...
ggsave(filename=paste0(fname,'_beta_1.pdf'), plot=q10, height=8.27, width=11.69)
