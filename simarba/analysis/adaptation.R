...
fixation_prob <- function(s,pop_size,mu,tau,pi) {
  ...
  res[pi!="SIM"] = 1-exp(-2*(1-(1-s)*exp(-tau*mu)))
  res[pi=="SIM"] = 1-exp(-2*(1-(1-s)*exp(-mu)))
  #res = res*exp(-tau*mu/s)
  return(res)
}

dt = fread("../adaptation_summary_2014-06-02.csv")
dt$real.tau = dt$tau
dt[pi==1000]$real.tau = 1

dtt = dt[phi==0 & rho==1, mean_se(final_tick), by="s,beta,pi,r,phi,rho,pop_size,envch_str,mu,num_loci,real.tau"]

dtt[,pi:=factor(dtt$pi,levels=c(0,1,1000),labels=c("CM","SIM","NM"))]
dtt[,phi:=factor(dtt$phi,levels=c(0,1,1000),labels=c("CR","SIR","NR"))]

dtt[, appearance_prob:= pop_size * beta * real.tau * mu / num_loci]
dtt[, fixation_prob:= fixation_prob(s,pop_size,mu,real.tau,pi)]
dtt[, fixation_time:= fixation_time(s,pop_size,1/pop_size,0.5)]
dtt[, adaptation_time:=envch_str*(fixation_time+1/(appearance_prob*fixation_prob))]
      
g=ggplot(data=dtt[r==0], mapping=aes(x=real.tau*mu, color=pi)) + 
  geom_point(aes(y=y/pop_size), size=2) + 
  geom_errorbar(aes(y=y/pop_size,ymax=ymax/pop_size, ymin=ymin/pop_size), width=0.3) +   
  facet_grid(pop_size~pi, labeller=tau_label, scales="free"  ) +
  geom_line(aes(y=adaptation_time/pop_size, linetype=pi)) + 
  scale_color_brewer("Mutator", palette="Set1", guide='none') +
  scale_y_log10() + scale_x_log10() + 
  labs(x="Mutational supply", y="Adaptation time") 
g

qplot(x=adaptation_time, y=y, data=dtt, color=factor(pi), size=I(3)) + labs(y="observed",x="expected",title="adaptation time") +
  geom_errorbar(aes(ymax=ymax,ymin=ymin)) +
  geom_abline(aes(slope=1, intercept=0)) + 
  facet_grid(envch_str~., scales="free") + scale_color_brewer("Strategy", palette="Set1") +
