...
fixation_prob <- function(s,pop_size,mu,tau,pi) {
  ...
  res[pi!=1] = 1-exp(-2*s)
  res[pi==1] = 1-exp(-2*(1-(1-s)*exp(-(tau-1)*mu)))
...
g=ggplot(data=dtt, mapping=aes(x=pop_size*real.tau, color=pi)) + 
...
g = ggplot(data=data,
  ...
  facet_grid(facets=envch_str+pop_size~real.tau, scales="free", labeller=tau_label) +
...
g = g + scale_color_brewer("Mutator", palette="Set1") +
  ...
  scale_shape_manual("Changes", values=c(16,2))
g
